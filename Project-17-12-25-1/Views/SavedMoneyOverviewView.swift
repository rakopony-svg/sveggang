        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            Chart {
                ...
                ForEach(viewModel.currentPeriodSavings, id: \.0) { entry in
                    ...
                    .interpolationMethod(.catmullRom)
                    .foregroundStyle(ThemeManager.shared.currentTheme.accent.opacity(0.2))
                }
            }
            .frame(height: 240)
        }
    ...
    private func statCard(title: String, value: String, icon: String) -> some View {
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(title).font(.caption).foregroundStyle(ThemeManager.shared.currentTheme.textSecondary)
                Text(value).font(.headline).foregroundStyle(ThemeManager.shared.currentTheme.textPrimary)
            }
            Spacer()
        }
        .padding()
